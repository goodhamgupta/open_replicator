...
public class OpenReplicator
{
	...
	private class ORBinlogParserListener implements BinlogParserListener
	{
		...
		public void onException(BinlogParser parser, Exception exception)
		{
			...
			notifyEventBinlogEventListenerOfException();
	...
	public void notifyEventBinlogEventListenerOfException()
	{
		LOGGER.info("Notifying event binlog listener of exception in Open-replicator");
		try
		{
			this.getBinlogEventListener().onException();
		}
		catch (Exception ex)
		{
			LOGGER.error("Error occured while notifying event binlog listener of exception. Exception:", ex);
		}
	...
	public void retry(BinlogParserContext context)
	{
		try
		{
			...
			this.binlogPosition = context.getCurrentPosition();
			LOGGER.info("Using the bin file name " + this.binlogFileName + " and position " + String.valueOf(this.binlogPosition) + " to retry.");
			dumpBinLog(this.binlogFileName, this.binlogPosition);
			this.binlogParser.start();
		}
		catch (Exception ex)
