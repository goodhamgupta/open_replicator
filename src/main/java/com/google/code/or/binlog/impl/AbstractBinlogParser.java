...
public abstract class AbstractBinlogParser implements BinlogParser
{
	...
	protected int checksumLength = 0;
	...
	protected class Context implements BinlogParserContext, BinlogEventListener
	{
		...
		public void onEvents(BinlogEventV4 event)
		{
			...
			{
				LOGGER.info("STOP_EVENT received, setting the next file name and position");
				LOGGER.info("Old values: binlog file name "+ this.binlogFileName + " and binlog position " + String.valueOf(this.currentPosition));
				//example of binlog file name "mysql-bin.000001"
				String newFileName= this.binlogFileName.substring(0,10);
				String binlog_num = this.binlogFileName.substring(10);
				Long bnum = Long.parseLong(binlog_num);
				bnum++;
				newFileName= newFileName + String.format("%06d", bnum);
				this.binlogFileName = newFileName;
				this.currentPosition = 4L;//default value
				LOGGER.info("New values: binlog file name "+ this.binlogFileName + " and binlog position " + String.valueOf(this.currentPosition));
			}
